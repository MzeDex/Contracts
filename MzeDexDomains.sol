...
contract MzeDexDomains is ERC721URIStorage { 
    ...
    constructor(string memory _tld) payable ERC721("MzeDex Name Service","MZE"){
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        if(len==3){
            return 350 * 10**17; // this equals to 35 Matic
        }else if(len==4){
            return 100 * 10**17; // this equals to 10 Matic
        }else{
            return 50 * 10**17; // this equals to 5 Matic
        }
    }
    ...
    function register(string calldata name) public payable{
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{ "name":"',_name,
                '","description": "A Domain name on MzeDex Name Service","image":"data:image/svg+xml;base64,',Base64.encode(bytes(finalSvg)),
                '","length" : "',strLen,
                '"}'
            )
        );   
